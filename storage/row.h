...
public:
	...
	RC init(uint64_t idx_key, table_t * host_table, uint64_t part_id, uint64_t row_id = 0, bool insert_row = false);
	void init(int size);
...
#if CC_ALG == MICA
#if !TPCC_CF
	static RC get_row(access_t type, txn_man * txn, table_t* table, row_t* access_row,
				   uint64_t row_id, uint64_t primary_key, void *row_head,  uint64_t part_id);
#else
