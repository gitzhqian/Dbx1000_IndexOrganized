...
#include "../util/roundup.h"
#include "../util/barrier.h"
#include "../util/lcore.h"
#include "../util/safe_cast.h"
...
void HugeTLBFS_SHM::initialize() {
  ...
  while (true) {
    ...
    *(size_t *)p=0;
    ...
    Page pg = Page{0, nullptr, nullptr, 0, false};
    pages_.push_back(pg);
...
//     printf("initial allocation of %zu on %p\n", kPageSize, p);
  ...
  for (page_id = 0; page_id < num_allocated_pages; page_id++) {
    ...
    pages_[page_id].paddr = (void*)((entry & 0x7fffffffffffffULL) * normal_page_size);
//     printf("virtual addr %p = physical addr %p\n", pages_[page_id].addr, pages_[page_id].paddr);
  }
