...
void Context<StaticConfig>::gc(bool forced) {
  ...
  auto gc_row = [this](ConcurrentTimestamp ts, Table<StaticConfig> *tbl, uint64_t cf_id, bool deleted,
                       uint64_t row_id, RowHead<StaticConfig> *head, RowVersion<StaticConfig> *write_rv) {
