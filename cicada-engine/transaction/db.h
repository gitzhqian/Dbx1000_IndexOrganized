...
#include "../common.h"
#include "timestamp.h"
#include "../alloc/hugetlbfs_shm.h"
#include "page_pool.h"
#include "table_mica.h"
#include "context.h"
...
struct BasicDBConfig {
  ...
  static constexpr bool kSkipPending = true;
 ...
 //  static constexpr uint64_t kInlineThreshold = 256 - 40;
  ...
  static constexpr size_t kMaxLCoreCount = 20;
  // The maximum number of numa nodes to support.
  static constexpr size_t kMaxNUMACount = 1;
...
class DB {
 public:
  ...
  HashIndexUniqueU64 *get_hash_index_unique_u64(std::string name) {
    return hash_idxs_unique_u64_[name];
  }
  HashIndexUniqueU64 *get_hash_index_unique_u64(std::string name) const {
    return hash_idxs_unique_u64_[name];
  }
  ...
  BTreeIndexUniqueU64 *get_btree_index_unique_u64(std::string name) const {
    return btree_idxs_unique_u64_[name];
  }
  ...
  BTreeIndexNonuniqueU64 *get_btree_index_nonunique_u64(std::string name) const {
    return btree_idxs_nonunique_u64_[name];
  }
 ...
 private:
  ...
  std::unordered_map<std::string, HashIndexNonuniqueU64*> hash_idxs_nonunique_u64_;
