...
// INSERT INTO HISTORY VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  ...
  uint64_t row_id;
  auto part_id = wh_to_part(w_id);
  if (!insert_row(_wl->t_history, row, part_id, row_id, 0)) return false;
...
//  index_read_range(index, key, max_key, rows, count, part_id);
  ...
  auto shared =  rows[0];
    itemid_t *item;
#if CC_ALG != MICA && !defined(EMULATE_SNAPSHOT_FOR_1VCC)
  auto local =  search(index, key, part_id, RD);
...
//  index_read_range(index, key, max_key, rows, count, part_id);
  ...
  for (uint64_t i = 1; i < count; i++) {
    auto shared = rows[i];
    itemid_t *item;
    key = orderlineKey(i, o_id, d_id, w_id);
#if CC_ALG != MICA && !defined(EMULATE_SNAPSHOT_FOR_1VCC)
    auto local =  search(index, key, part_id, RD);
    //auto local = get_row(index, reinterpret_cast<row_t *>(shared), item, part_id, RD);
#else
...
RC tpcc_txn_man::run_order_status(tpcc_query* query) {
 ...
 // else
  ...
  auto oid = order_status_getOrderId(arg.w_id, arg.d_id, c_id);
  int64_t o_id = oid;
  //auto order = order_status_getLastOrder(n_o_id, arg.w_id, arg.d_id, c_id);
//  if (order != NULL) {
//    int64_t o_id;
...
//  index_read_range(index, key, max_key, rows, count, part_id);
  ...
  for (uint64_t i = 1; i < count; i++) {
    auto shared = rows[i];
    itemid_t *item;
    key = orderlineKey(i, no_o_id, d_id, w_id);
#if !TPCC_CF
#if CC_ALG == MICA
    auto local = get_row(index, *reinterpret_cast<uint64_t *>(shared), key, part_id, WR);
#else
    auto local = search(index, key, part_id, WR);
    //auto local = get_row(index, reinterpret_cast<row_t *>(shared), item, part_id, WR);
#endif
...
// DBx1000's active delivery transaction checking.
...
#endif
  {
    ...
    if (!delivery_getNewOrder_deleteNewOrder(n_o_oid, d_id, arg.w_id, &o_id)) {
      // printf("oops0\n");
      FAIL_ON_ABORT();
#ifdef TPCC_DBX1000_SERIAL_DELIVERY
...
bool tpcc_txn_man::stock_level_getStockCount(uint64_t ol_w_id, uint64_t ol_d_id,
  ...
  void* rows[301];
  uint64_t count = 301;
  uint64_t ol_count = 6; //[5,15]
  uint64_t ol_o_id_count = 3000; //[0,3000]
...
//  index_read_range(index, key, max_key, rows, count, part_id);
  ...
  //ol_o_id, for each order
  //ol_count, for each order line
  for (uint64_t i = 1; i < 20; i++) {
      for (int ol_i = 1; ol_i < ol_count; ++ol_i) {
          auto orderline_shared = rows[i];
          itemid_t *item;
          key = orderlineKey(ol_i, ol_o_id - i, ol_d_id, ol_w_id);
#if CC_ALG != MICA && !defined(EMULATE_SNAPSHOT_FOR_1VCC)
          auto orderline = search(index, key, part_id, RD);
...
//          if (orderline == NULL) return false;
          if (orderline == NULL) {
              continue; //this oderline may insert fail;
          };
